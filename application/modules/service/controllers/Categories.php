...
public function delete($id) {
  ...
  $dt = $this->common_model->view_data("service_categories", array("id" => $id), "id", "asc");
 
 
  if ($dt) {
 
      $this->common_model->delete_data("service_categories", array("id" => $id),$id);
     ...
     $this->session->set_flashdata('error', 'An error occurred. Please try again.');
      ...
      redirect(base_url() . "service/categories", "refresh");
...
public function update(){
  ...
  $selPdt=$this->common_model->view_data("service_categories",array("id"=>$id),"id","desc");
 
  $data = array(
    "name"                       => $this->common_model->xss_clean($this->input->post("ename")),   
    "amount"                     => $this->common_model->xss_clean($this->input->post("eamount")),
    "is_active"                  => $this->common_model->xss_clean($this->input->post("is_active")),
                  
      );

      
      if ($this->common_model->update_data("service_categories", $data,array("id"=>$id),$id)) {
